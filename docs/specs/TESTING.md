...
- [ ] DnD/리사이즈(반복) → 드래그한 회차 DnD 종료 위치 유지 → 범위 선택 → 저장 중에도 드래그 회차 고정·나머지 refetch 후 이동 → 취소/실패 시 `revert`
...
- [ ] 「오늘 일정」「이번 주 일정」 등 자연어 조회 → 채팅 요약 + 상단 **「조회된 일정 N건」**
- [ ] 반복 일정이 **회차 단위**로 표시되고, 제외한 회차는 결과에 없음 (달력과 일치)
- [ ] 20건 초과 시 **「더보기」** → +20건 추가 (LLM 재호출 없이)
- [ ] 시간대 필터: 「이번 주 저녁 일정」 등
- [ ] 특정 날짜: 「30일 일정」(이번 달), 「6월 30일」 등
- [ ] 조회 항목 클릭 → 수정 Modal

### AI — 추가·수정 (1·3차)

- [ ] 일정 추가 후 상단 **「추가된 일정 N건」**, 클릭 시 Modal
- [ ] 개별 일정 수정 → **「수정된 일정 N건」** (되묻기 없음)
