...
> 상세 기능 명세: [docs/specs/README.md](docs/specs/README.md)

- **달력**: FullCalendar 기반 월/주/일/목록 뷰, 드래그 생성·이동·크기 조절, 반복 일정
- **AI 일정 관리**: 자연어로 일정 추가/수정/삭제 (반복 범위 선택·되묻기 포함)
- **AI 일정 조회**: 기간·시간대·키워드 자연어 조회, 반복 회차 전개, 더보기
- **음성 입력**: Web Speech API (Chrome/Edge), 인식 후 입력창 확인·전송
...
2. SQL Editor에서 `supabase/migrations/` 아래 파일을 **번호 순**으로 실행  
   (`001` → `007`. `006`·`007`은 기존 DB에 1회만 적용 — [docs/specs/TIME.md](docs/specs/TIME.md))
...
수동 QA 체크리스트: [docs/specs/TESTING.md](docs/specs/TESTING.md) §7
...
- 마이크 버튼 클릭 → 말하기 → **인식 결과가 입력창에 채워짐** → 확인·수정 후 전송
